...
// Render challenges - UPDATED to handle all challenge types
function renderChallenges(stage, stageProgress) {
    ...
    stage.challenges.forEach((challenge, index) => {
        ...
        // Add special styling for programming challenges
        const programmingTypes = ['cipher', 'code_puzzle', 'hidden_code', 'binary', 'hex', 
                                  'location_puzzle', 'json_puzzle', 'ascii_puzzle', 
                                  'sequence_puzzle', 'color_code', 'final_riddle'];
        if (programmingTypes.includes(challenge.type)) {
            challengeDiv.setAttribute('data-type', challenge.type);
        }
        
        challengeDiv.style.opacity = isCompleted ? '0.6' : '1';
        
        // Format question with proper code blocks
        let formattedQuestion = challenge.question;
        if (challenge.question.includes('\n')) {
            formattedQuestion = challenge.question.replace(/\n/g, '<br>');
        }
        
        let challengeHTML = `
            <h3>${isCompleted ? '✓ ' : ''}${getChallengeName(challenge.type)} ${index + 1} ${isCompleted ? '(مكتمل)' : ''}</h3>
            <p style="font-size: 1.1rem; margin: 1rem 0; white-space: pre-wrap;">${formattedQuestion}</p>
        ...
        if (!isCompleted) {
            if (challenge.type === 'quiz') {
                ...
                // All other types use text input
                let placeholder = 'أدخل إجابتك هنا...';
                if (challenge.type === 'binary') placeholder = 'أدخل الرقم العشري...';
                if (challenge.type === 'hex') placeholder = 'أدخل الرقم العشري...';
                if (challenge.type === 'cipher') placeholder = 'أدخل النص المفكوك...';
                if (challenge.type === 'code_puzzle') placeholder = 'أدخل الناتج الحسابي...';
                if (challenge.type === 'hidden_code') placeholder = 'أدخل الكود السري...';
...
// Get challenge name by type
function getChallengeName(type) {
    const names = {
        'quiz': 'السؤال',
        'puzzle': 'اللغز',
        'cipher': '🔐 تحدي التشفير',
        'code_puzzle': '💻 تحدي البرمجة',
        'hidden_code': '🕵️ تحدي الكود المخفي',
        'binary': '🔢 تحدي Binary',
        'hex': '🔣 تحدي Hexadecimal',
        'location_puzzle': '🗺️ تحدي الموقع',
        'json_puzzle': '📊 تحدي JSON',
        'ascii_puzzle': '🔤 تحدي ASCII',
        'sequence_puzzle': '🔢 تحدي المتتالية',
        'color_code': '🎨 تحدي الألوان',
        'final_riddle': '🏆 اللغز النهائي'
    };
    return names[type] || 'التحدي';
}

// Check answer - UPDATED to handle all challenge types
